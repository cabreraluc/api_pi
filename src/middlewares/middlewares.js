...
async function SubirPokemonesABd (){
    ...
    const b = await a.json()
       ...
       b.results.forEach( async (element)=> {
        const busquedaIndividual = await Pokemon.findOne({where:{name:element.name}})
        const busquedaIndividual2 = await Deleted.findOne({where:{name:element.name}})
...
restoreFromTrash: async (lista)=>{
    ...
    lista.forEach( async (element) => {
        const pokemonEnCuestion = await Deleted.findOne({where:{name:element}})
        console.log(element)
        while (!pokemonEnCuestion) {
         
        }
        Deleted.destroy({where:{name:element}})
        const pokemonEnCuestionCreate = await Pokemon.create(pokemonEnCuestion.dataValues)
    })
